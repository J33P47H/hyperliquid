...
def generate_parameter_combinations(param_config, max_combinations: int = 100) -> List[Dict[str, Any]]:
    ...
    Handles both range lists and start/end/step configurations.
    ...
    for param_name, param_info in param_config.items():
        if isinstance(param_info, dict):
            ...
            if 'range' in param_info:
            ...
            elif all(k in param_info for k in ['start', 'end', 'step']):
                # Generate range from start, end, step
                ...
                if isinstance(start, int) and isinstance(end, int) and isinstance(step, int):
                    ...
                    # For float parameters, use numpy arange and round to avoid floating point issues
                    values = [round(x, 3) for x in np.arange(start, end + step/2, step)]

...
def generate_parameter_combinations(strategy_class, max_combinations: int = 100) -> List[Dict[str, Any]]:
...
def save_results(stats, strategy_name, symbol, timeframe, params=None, output_dir=RESULTS_DIR):
    ...
    try:
        ...
        results_dict = {
            'Start': stats['Start'].strftime('%Y-%m-%d %H:%M:%S') if isinstance(stats['Start'], pd.Timestamp) else stats['Start'],
            'End': stats['End'].strftime('%Y-%m-%d %H:%M:%S') if isinstance(stats['End'], pd.Timestamp) else stats['End'],
        ...
        if filename.exists():
            try:
                ...
                # If there's an error reading existing file, create new one
...
def optimize_strategy(df, strategy_name, strategy_class, strategy_config, backtest_settings):
    ...
    try:
        # Generate parameter combinations from strategy class
        ...
        if not param_combinations:
            print(f"Warning: No parameter combinations found for {strategy_name}")
                    ...
                    try:
                        ...
                        if strategy_info.get('optimize', False):
                            ...
                            default_params = {
                                ...
                                for k, v in strategy_info['params'].items()
                            }
                            
                            # Run single backtest with default parameters
                            stats, bt = run_strategy_backtest(
                                df,
                                strategy_class,
                                default_params,
